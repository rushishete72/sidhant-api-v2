...
const validateLocationCreation = (data) => {
  ...
  if (
    !tr(location_code) ||
    tr(location_code).length < 2 ||
    tr(location_code).length > 20
  ) {
...
const validateLocationUpdate = (data) => {
  if (Object.keys(data).length === 0)
    return "अपडेट करने के लिए कम से कम एक फ़ील्ड प्रदान करें।";
  ...
  if (
    ...
    (!tr(location_name) || tr(location_name).length > 50)
  ) {
...
// (अपरिवर्तित Exports)
const validateUomCreation = () => {
  return null;
};
const validateUomUpdate = () => {
  return null;
};
...
const validateQCLotCreation = () => {
  return null;
};
const validateQCLotUpdate = () => {
  return null;
};
const validateQcPlanCreationOrUpdate = () => {
  return null;
};
